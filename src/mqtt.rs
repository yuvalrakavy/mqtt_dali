...
impl <'a> MqttDali<'a> {
    ...
    fn get_light_reply_topic(&self, command: &str, bus: usize, short_address: u8) -> String {
        format!("DALI/Reply/{}/{}/Bus_{}/Address_{}", command, self.dali_config.name, bus, short_address)
    }

    async fn publish_config(client: &AsyncClient, config_topic: &str, dali_config: &DaliConfig) -> Result<()> {
        Ok(client.publish(config_topic, QoS::AtLeastOnce, true, serde_json::to_vec(dali_config)?).await?)
    }
    ...
    async fn query_light_status(&mut self, bus: usize, short_address: u8) -> Result<DaliBusResult> {
        ...
        let query_light_reply = match light_status {
            Ok(light_status) => QueryLightReply::new(&self.dali_config.name, bus, short_address, light_status),
            Err(e) => QueryLightReply::new_failure(&self.dali_config.name, bus, short_address, e),
        };
    ...
    async fn find_lights(&mut self, config_topic: &str, bus_number: usize, selection: DaliDeviceSelection) -> Result<DaliBusResult> {
        ...
        while device_iterator.find_next_device(self.dali_manager)?.is_some() {
            let short_address = (0..64u8).find(|short_address|
                !self.dali_config.buses[bus_number].channels.iter().any(|channel| channel.short_address == *short_address)
