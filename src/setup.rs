...
impl BusConfig {
    ...
    pub fn display(&self) {
        println!("{}: DALI bus: {} ({})", self.bus+1, self.description, self.status);
...
impl Setup {
    pub fn assign_addresses(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize) -> Result<DaliConfig, Box<dyn std::error::Error>> {
                ...
                match command {
                    ...
                    'm' => {
                        ...
                        while dali_bus_iterator.find_next_device(dali_manager)?.is_some() {
                            ...
                            let description = Setup::prompt_for_string("Description",Some(&format!("Light {}", short_address)))?;
                            ...
                            if let Some(index) = dali_config.buses[bus_number].get_channel_index(short_address) {
                                if let Ok(new_short_address) = Setup::prompt_for_short_address("To address", &None) {
                                    ...
                                    if new_short_address != short_address {
                                        if dali_config.buses[bus_number].find_member(new_short_address).is_some() {
                                            println!("Short address is already used");
                                        }
                                        else {
                                            let mut dali_bus_iterator = 
                                                DaliBusIterator::new(dali_manager, bus_number , DaliDeviceSelection::Address(short_address), None).expect("Error while initializing DALI bus iteration");
                                            let mut done = false;
                                            ...
                                            while dali_bus_iterator.find_next_device(dali_manager)?.is_some() {
                                                if !done {
                                                    ...
                                                    dali_config.buses[bus_number].channels[index].short_address = new_short_address;     // Update configuration
                                                    done = true;
                                                    config.save(&dali_config)?;
                                                } else {
    ...
    fn delete_group(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize, group_address: u8) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        //let bus_config = &mut dali_config.buses[bus_number];
    ...
    fn new_group(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize, group_address: u8) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        //let bus_config = &mut dali_config.buses[bus_number];

        let description = Setup::prompt_for_string("Description", Some(&format!("Group {}", group_address)))?;
        dali_config.buses[bus_number].groups.push(Group { description, group_address, members: Vec::new() });
        dali_config = Setup::edit_group(config, dali_config, dali_manager, bus_number, group_address)?;
        config.save(&dali_config)?;
        Ok(dali_config)
    }

    fn edit_group(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize, group_address: u8) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        //let bus_config = &mut dali_config.buses[bus_number];
                    ...
                    match command {
                        'a' => {
                            ...
                            if dali_config.buses[bus_number].get_channel_index(short_address).is_none() {
                                ...
                                if let Err(err) = dali_manager.add_to_group_and_verify(bus_number, group_address, short_address) {
                        ...
                        'd' => {
                            ...
                            if let Some(index) = index {
                                ...
                                dali_manager.remove_from_group_and_verify(bus_number, group_address, short_address)?;
                                config.save(&dali_config)?;
                            }
                            else {
                        ...
                        'p' => {
                            let light_name_pattern = Setup::prompt_for_string("Group members are lights whose names match", None)?;

                            dali_manager.match_group(&mut dali_config.buses[bus_number], group_address, &light_name_pattern, Some(Box::new(|action, _| {
        ...
        for light in bus_config.channels.iter() {
            match dali_manager.query_group_membership(bus_config.bus, light.short_address) {
                Ok(group_mask) => {
                    ...
                    let mut mask = 1u16;
                    for group_number in 0..16 {
                        if (group_mask & mask) != 0 && !bus_config.groups.iter().any(|g| g.group_address == group_number) {
                            println!("Light {} is member of group {} which is not in configuration:", light.short_address, group_number);
                            match dali_manager.remove_from_group_and_verify(bus_config.bus, group_number, light.short_address) {
                    ...
                    for group in bus_config.groups.iter() {
                        ...
                        if group.members.iter().any(|m| light.short_address == *m) && (group_mask & mask) == 0 {
                            ...
                            match dali_manager.add_to_group_and_verify(bus_config.bus, group.group_address, light.short_address) {
                                Ok(_) => println!("  added!"),
                                Err(e) => println!(" error: {}", e),
                            }
                        }
    ...
    pub fn interactive_setup_groups(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            let command = Setup::prompt_for_string("Groups: n=new, d=delete, e=edit, s=set-level, f=fix, b=back", Some("b"))?;

            if let Some(command) = command.chars().next() {
                match command {
                    'b' => return Ok(dali_config),
                    'n' => {
                        if let Some(group_address) = Setup::prompt_for_new_group_address(&dali_config.buses[bus_number], "Add group")? {
                            dali_config = Setup::new_group(config, dali_config, dali_manager, bus_number, group_address)?;
                        }
                    },
                    's' => {
                    ...
                    'd' => {
                        if let Some(group_address) = Setup::prompt_for_existing_group_address(&dali_config.buses[bus_number], "Delete group", None)? {
                            dali_config = Setup::delete_group(config, dali_config, dali_manager, bus_number, group_address)?;
                        }
                    },
                    'e' => {
                        if let Some(group_address) = Setup::prompt_for_existing_group_address(&dali_config.buses[bus_number], "Edit group", None)? {
                            dali_config = Setup::edit_group(config, dali_config, dali_manager, bus_number, group_address)?;
                        }

                    },
                    'f' => Setup::fix_group_membership(&dali_config.buses[bus_number], dali_manager),
                    _ => println!("Invalid command"),
                }
            }
        }
    ...
    fn fix_config(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        ...
        if all_lights_ok {
            println!("All lights were found ({})", dali_config.buses[bus_number].channels.len());
        } else {
            for short_address_to_remove in remove_list.iter() {
                if let Some(index) = dali_config.buses[bus_number].channels.iter().position(|l| l.short_address == *short_address_to_remove) {
    ...
    fn interactive_setup_lights(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        //let mut bus_config = &mut dali_config.buses[bus_number];
            ...
            if let Some(command) = command.chars().next() {
                match command {
                    ...
                    'r' => {
                        ...
                        if let Some(short_address) = Setup::prompt_for_existing_short_address( bus_config, "Rename", last_short_address)? {
                            let index =  bus_config.get_channel_index(short_address).unwrap();
                            let description = Setup::prompt_for_string("Description: ", Some(& bus_config.channels[index].description))?;
                    ...
                    'q' => {
                        if let Some(short_address) = Setup::prompt_for_existing_short_address( &dali_config.buses[bus_number], "Address", last_short_address)? {
                            dali_config.buses[bus_number].do_query_light(dali_manager,short_address);
                        }
                    },
                    'g' => {
                        if let Some(short_address) = Setup::prompt_for_existing_short_address( &dali_config.buses[bus_number], "Address", last_short_address)? {
                            let mask = dali_manager.query_group_membership(bus_number, short_address)?;
                            println!("Light {bus_number}/{short_address} Group membership mask: {mask}");
                        }
                    }
                    '?' =>  dali_config.buses[bus_number].display(),
                    _ => println!("Invalid command"),
    ...
    pub fn interactive_bus_setup(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager, bus_number: usize) -> Result<DaliConfig, Box<dyn std::error::Error>> {
        //let mut bus_config = &mut dali_config.buses[bus_number];

        if !matches!(dali_config.buses[bus_number].status, BusStatus::Active) {
            loop {
                dali_config.buses[bus_number].display();
                println!("Bus {}!", if matches!(dali_config.buses[bus_number].status,BusStatus::NoPower) { "has no power"} else { "is overloaded"});
                ...
                if let Some(command) = command.chars().next() {
                    match command {
                        'b' => return Ok(dali_config),
                        'r' => {
                            dali_config.buses[bus_number].description = Setup::prompt_for_string("Description", Some(&dali_config.buses[bus_number].description))?;
                            config.save(&dali_config)?;
                        },
                        _ => println!("Invalid command"),
                    }
            ...
            loop {
                ...
                if let Some(command) = command.chars().next() {
                    match command {
                        ...
                        'a' => dali_config = Setup::assign_addresses(config, dali_config, dali_manager, bus_number)?,
                        'g' => dali_config = Setup::interactive_setup_groups(config, dali_config, dali_manager, bus_number)?,
                        'l' => dali_config = Setup::interactive_setup_lights(config, dali_config, dali_manager, bus_number)?,
                        'q' => dali_config.buses[bus_number].query_bus(dali_manager),
                        'f' => dali_config = Setup::fix_config(config, dali_config, dali_manager, bus_number)?,
                        _ => println!("Invalid command"),
                    }
                }
            }
    ...
    pub fn interactive_setup(config: &Config, mut dali_config: DaliConfig, dali_manager: &mut DaliManager) -> Result<SetupAction, Box<dyn std::error::Error>> {
        ...
        loop {
            ...
            if let Some(command) = command.chars().next() {
                match command {
                    ...
                    'r' => {
                        dali_config.name = Setup::prompt_for_string("Name", Some(&dali_config.name))?;
                    },
                    'b' => {
                        ...
                        if bus_number >= dali_config.buses.len() {
                            ...
                            dali_config = Setup::interactive_bus_setup(config, dali_config, dali_manager, bus_number)?;
                        }
                    },
                    _ => println!("Invalid command"),
                }
